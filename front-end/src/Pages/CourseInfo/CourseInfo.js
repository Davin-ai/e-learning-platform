...
export default function CourseInfo() {
  ...
  const [sessions, setSsions] = useState([])
  ...
  const getCourseDetails = () => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/v1/courses/${courseName}`, {
      ...
      .then((courseInfo) => {
        console.log('fetch comment :', courseInfo.comments)

        setComments(courseInfo.comments)
        setSsions(courseInfo.sessions)
  ...
  const registerInCourse = (course) => {
    ...
    if (course.price === 0) {
      ...
      swal({
        title: "ایا از ثبت نام در دوره مطمئن هستید؟",
        icon: "warning",
        buttons: ["خیر", "اره"],
      }).then((result) => {
        if (result) {
  ...
  return (
    <>
      ...
      <Breadcrumb
        links={[
          { id: 1, title: 'خانه', to: '' },
          { id: 2, title: 'آموزش برنامه نویسی فرانت اند', to: 'category-info/:categoryName' },
          { id: 3, title: 'دوره متخصص جاوا اسکریپت', to: 'course-info/:courseName' },
        ]}
      />
                              ...
                              (session.free === 1 || courseDetails.isUserRegisteredToThisCourse) ? (
                                <>
                                  <div className="introduction__accordion-right">
                                    ...
                                    <i className="fab fa-youtube introduction__accordion-icon you-tube"></i>
